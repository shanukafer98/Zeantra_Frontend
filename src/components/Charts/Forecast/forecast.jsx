...
const Forecast = (props) => {
  ...
  useEffect(() => {
    const fetchDateRange = async () => {
      const response = await axios.get(
        `http://127.0.0.1:8000/ValueCounts/FullTimePeriod_Frontend`
      );
  ...
  const pastData = Object.keys(sales_or_profit_data)
    .map(Number)
    .map((key) =>
      key <= 47 ? parseFloat(sales_or_profit_data[key]).toFixed(2) : null
    );
  const forecastData = Array(48)
    .fill(null)
    .concat(
      Object.keys(sales_or_profit_data)
        .map(Number)
        .filter((key) => key >= 48)
        .map((key) => parseFloat(sales_or_profit_data[key]).toFixed(2))
    );
  ...
  const options = {
    ...
    xaxis: {
      categories: Array.from(
        { length: Object.keys(sales_or_profit_data).length },
        (_, i) => indexToYearMonth(i)
      ),
      labels: {
        style: {
          colors: "#708090",
  ...
  return (
    <>
      <div className="overflow-x-auto ">
        <Chart options={options} series={series} type="line"  width="1100"/>
      </div>
      <h3 className="text-blue-700 text-2xl text-center my-3 font-extrabold">
        Get forcast based on time frames
      </h3>
